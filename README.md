...
- **Overview** - stage counts across the pipeline, a 30-day activity sparkline with week-over-week momentum, an interview-rate bar, a "Sector focus" breakdown of tracked applications, a status-breakdown donut, a scrape-activity heatmap, and what needs your attention (including upcoming interviews and stale drafts/applications worth a follow-up)
- **Discovery** - every scraped posting, sortable/filterable, with location and salary columns and a "Why?" drawer showing both `/scrape`'s own reasoning (highlights, referral links) and `/rank`'s (strengths, gaps, deadline, location veto) instead of just a bare score; a posting you've already drafted or applied to shows its real tracked status instead of a bare "Apply" button
- **Pipeline** - your tracked applications as a six-stage board (`Drafted` through `Rejected/Closed`), with a drawer showing `status`/`notes` (editable here), plus the next interview date, CV/cover-letter file paths, and any interview prep/cheat-sheet files - `/apply` creates a row the moment it drafts something, `/interview` adds the next interview date, and `/outcome` carries it the rest of the way
- **Upskill** - a read view over the framework's own generated skill-gap reports
- **Salary** - browse the framework's benchmark data (with its Python-backed lookup), and add, edit, or delete benchmarked companies and index metadata directly
...
- **Settings** - edit what `/scrape` searches for and which tool calls auto-approve, see which portal-search skills are installed (with a per-portal health indicator) and which custom CV/cover-letter templates are registered and active
...
| `job_search_tracker.csv`                                                                                        | Overview, Pipeline                         | Editing a row's status/notes from the Pipeline drawer. Every other column - including `next_interview_date`, added by `/interview` and cleared by `/outcome` on resolution - is written only by the framework's own commands (`/apply` creates the row when it drafts something; `/interview` and `/outcome` update it from there) and read-only here. |
| `documents/applications/*/outcome.md`                                                                           | Pipeline (interview stages, outcome notes) | never                                                                                                                                                                                                                                                                                                                                                  |
| `upskill/report-*.md`                                                                                           | Upskill page                               | never                                                                                                                                                                                                                                                                                                                                                  |
| `salary_data.json`                                                                                              | Salary page                                | Adding, editing, or deleting a benchmarked company (Salary page's company drawer), and editing the index metadata (source/baseline/label)                                                                                                                                                                                                              |
...
| `job_scraper/portal_health.json`                                                                                | Settings page (Installed portals)          | never - read-only per-portal result count/status; `/scrape` writes it after every run                                                                                                                                                                                                                                                                  |
| `templates/**/TEMPLATE.md` + the `ACTIVE-TEMPLATE` block in `05-cv-templates.md`/`06-cover-letter-templates.md` | Settings page (Active templates)           | never - read-only; `/add-template` registers and activates templates                                                                                                                                                                                                                                                                                   |
