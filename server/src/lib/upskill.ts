...
export function parseUpskillReport(
  filename: string,
  rawText: string,
): UpskillReport {
  ...
  const gapHeatmap: GapHeatmapRow[] = heatmapSection
    ? parseMarkdownTableRows(heatmapSection).map(
        ([priority, skill, type, gapSource]) => ({
          priority: priority ?? "",
          skill: skill ?? "",
          type: type ?? "",
          gapSource: gapSource ?? "",
  ...
  const suggestedStudyOrder: StudyOrderRow[] = studySection
    ? parseMarkdownTableRows(studySection).map(
        ([order, topic, type, estTime, note]) => ({
          order: order ?? "",
          topic: topic ?? "",
          type: type ?? "",
          estTime: estTime ?? "",
...
export async function listUpskillReports(): Promise<UpskillReport[]> {
  ...
  const filenames = entries
    .filter(
      (e) =>
        e.isFile() && e.name.startsWith("report-") && e.name.endsWith(".md"),
    )
    .map((e) => e.name)
    .sort()
    .reverse();
  ...
  return Promise.all(
    filenames.map(async (filename) => {
      const text = await readFile(
        path.join(paths.upskillDir, filename),
        "utf-8",
      );
